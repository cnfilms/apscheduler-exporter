...
class APSchedulerExporter:
    def __init__(self, scheduler, registry: Optional[CollectorRegistry] = None):
        ...
        self.scheduler.add_listener(self._on_job_started, EVENT_JOB_ADDED | EVENT_JOB_SUBMITTED)
    ...
    def _on_job_started(self, event: JobSubmissionEvent):
        if event.job_id not in self._jobs_cache:
            if job := self.scheduler.get_job(event.job_id):
                self._jobs_cache[event.job_id] = {"job_name": job.name, "start_time": time.time()}

    def _on_job_terminated(self, event: JobExecutionEvent):
        if job_data := self._jobs_cache.get(event.job_id):
            self.event_job_metrics[event.code].labels(job_name=job_data["job_name"]).inc()
            self.last_job_duration_metrics.labels(job_name=job_data["job_name"]).set(time.time() - job_data["start_time"])
